CREATE OR REPLACE VIEW bi_alefdw_dev.core_course_ic_classlearning_agg_view AS
...
FULL OUTER JOIN bi_alefdw.class_total_students_prev_ay_mv cts_prev
...
core_course_ic_experience AS (
    ...
    FROM bi_alefdw.students_ic_progress_mv
    GROUP BY 1,2,3
    UNION ALL
    SELECT *
    FROM bi_alefdw.core_course_ic_experience_mv ccfle
    WHERE NOT EXISTS(SELECT 1 FROM bi_alefdw.students_ic_progress_mv slp WHERE ccfle.fle_class_dw_id = slp.fle_class_dw_id)
)
SELECT cont.*,
       cts.section_dw_id,
       cts.section_name,
       ...
       fact.session_time
FROM bi_alefdw.core_class_ic_content_mv cont
