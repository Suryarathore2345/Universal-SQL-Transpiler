CREATE MATERIALIZED VIEW bi_coredw_dev.fact_learning_experience_silver_mv AS
...
FROM coredw.fact_learning_experience fle
         JOIN coredw.dim_learning_objective lo
              ON lo.lo_dw_id = fle.fle_lo_dw_id
                  AND lo.lo_status = 1
         JOIN bi_coredw.bi_all_schools_dim_mv sch
         ...
         JOIN (SELECT class_dw_id,
               ...
               FROM bi_coredw.core_class_activity_content_mv GROUP BY 1) cont
...
WHERE fle_abbreviation <> 'NA'
  ...
  AND fle_ls_id NOT IN (SELECT DISTINCT fle_ls_id FROM coredw.fact_learning_experience WHERE fle_state = 4)
  AND EXISTS (SELECT 1 FROM bi_coredw.core_class_activity_content_mv cont2
