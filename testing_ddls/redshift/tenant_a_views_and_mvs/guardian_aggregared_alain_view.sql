CREATE OR REPLACE VIEW bi_coredw_dev.guardian_aggregared_alain_view AS
(
                       ...
                       FROM (SELECT DISTINCT DATE_TRUNC('week', full_date
                             ...
                             WHERE week_start_date <= CURRENT_DATE
                               AND full_date >= (SELECT min(activity_date)
                                                 FROM bi_coredw.guardian_activity_dm_view
                                                 WHERE academic_year is not null)) a),
     max_ay AS (Select school_dw_id, max(academic_year) as max_ay from bi_coredw.guardian_activity_dm_view group by 1)
...
FROM calendar_year cy
         CROSS JOIN bi_coredw.guardian_activity_dm_view ga
