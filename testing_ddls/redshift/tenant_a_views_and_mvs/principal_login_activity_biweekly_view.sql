CREATE OR REPLACE VIEW bi_coredw_dev.principal_login_activity_biweekly_view as
WITH total_principals as (SELECT DISTINCT ds.*,
                          ...
                          FROM bi_coredw.bi_active_schools_dim_mv ds
                                   CROSS JOIN (SELECT distinct full_date
                                               ...
                                               WHERE dt.full_date between trunc(sysdate) - 14 and trunc(sysdate)) date_dateset
                                   INNER JOIN coredw.dim_staff_user_school_role_association dsusra
                                              on dsusra.susra_school_dw_id = ds.school_dw_id
                                                  AND dsusra.susra_status = 1
                                   INNER JOIN coredw.dim_staff_user dp
                                          ...
                                          INNER JOIN coredw.dim_staff_user dp1
                                              ON dp1.staff_user_dw_id = dsusra.susra_staff_dw_id
                                                  ...
                                                  AND (((staff_user_status = 2 AND staff_user_enabled is TRUE)
                                                      ...
                                                      OR (staff_user_status = 1 AND staff_user_enabled is TRUE))
                                 WHERE Trunc(login_local_date_time) between trunc(sysdate) - 14 and trunc(sysdate)) prin_dateset),
     principal_onboarding as (SELECT DISTINCT principal_dw_id,
                              ...
                              FROM bi_coredw.principal_login tl
                                       INNER JOIN bi_coredw.bi_active_schools_dim_mv ds
