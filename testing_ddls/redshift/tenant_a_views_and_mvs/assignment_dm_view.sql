create or replace view bi_coredw_dev.assignment_dm_view as
with _teachers as
     ...
     fact_assignment_submission AS ( -- getting student' same assignment submission last status
         ...
         FROM (SELECT *,
               ...
               FROM coredw.fact_assignment_submission) sub
         WHERE sub.rank = 1)
...
FROM coredw.dim_assignment da
         LEFT JOIN coredw.dim_assignment_instance dai
                   ON dai.assignment_instance_assignment_dw_id = da.assignment_dw_id
         LEFT JOIN coredw.dim_assignment_instance_student dais
        ...
        INNER JOIN coredw.dim_course_subject_association dcsa
         ...
         INNER JOIN bi_coredw.bi_active_schools_dim_mv as dsc
         ...
         INNER JOIN coredw.dim_grade as dg
