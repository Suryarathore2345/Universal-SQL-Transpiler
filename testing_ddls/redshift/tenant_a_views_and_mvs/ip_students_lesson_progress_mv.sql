create materialized view bi_coredw_dev.ip_students_lesson_progress_mv AS
WITH COMPLETED_LESSONS AS (select *
                           from (select fle_ls_id,
                                 ...
                                 FROM coredw.fact_learning_experience
                                          ...
                                          JOIN coredw.dim_academic_year ay
     ...
     student_lessons_assigned AS ( -- lesson level learning plan for each student
         ...
         FROM coredw.dim_class_user d_cu
                  ...
                  INNER JOIN coredw.dim_instructional_plan dip
...
FROM (SELECT DISTINCT dd.full_date                                                AS local_date,
      ...
      FROM coredw.fact_learning_experience fle
               JOIN coredw.dim_term fle_dtrm ON fle.fle_term_dw_id = fle_dtrm.term_dw_id
               JOIN bi_coredw.bi_student_dim_mv dst
                    ON fle.fle_student_dw_id = dst.student_dw_id AND student_status = 1
               JOIN coredw.dim_date dd ON fle.fle_date_dw_id = dd.date_id
      ...
      WHERE fle_abbreviation <> 'NA'
        ...
        AND fle_material_type <> 'PATHWAY'
        AND
          fle.fle_ls_id NOT IN (select distinct fle_ls_id from coredw.fact_learning_experience where fle_state = 4)) fl
