...
CREATE OR REPLACE VIEW bi_coredw_dev.student_learning_feats_view AS
...
with  class_db as (select class_dw_id,
     ...
     db as (select *
            from coredw.fact_learning_experience
                   join coredw.dim_student on fle_student_dw_id = student_dw_id
                   join bi_coredw.bi_active_schools_dim_mv on student_school_dw_id = school_dw_id
                   join coredw.dim_grade on student_grade_dw_id = grade_dw_id
                   join class_db on fle_class_dw_id = class_dw_id and id = 1
                   left join coredw.dim_interim_checkpoint on fle_lo_dw_id = ic_dw_id
