CREATE MATERIALIZED VIEW bi_coredw_dev.students_lesson_progress_mv AS
...
FROM (SELECT DISTINCT TRUNC(CONVERT_TIMEZONE('UTC', dsc.tenant_timezone, fle.fle_created_time)) AS local_date,
      ...
      WHERE fle_abbreviation <> 'NA'
        ...
        AND fle.fle_ls_id NOT IN
            (select distinct fle_ls_id from coredw.fact_learning_experience where fle_state = 4)) fl
