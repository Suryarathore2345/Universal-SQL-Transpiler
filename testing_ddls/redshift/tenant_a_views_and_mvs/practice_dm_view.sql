CREATE OR REPLACE VIEW bi_coredw_dev.practice_dm_view
AS
SELECT distinct dsc.school_name,
                ...
                dsc.school_id
FROM coredw.fact_practice fp
         JOIN bi_coredw.bi_active_schools_dim_mv dsc ON dsc.school_dw_id = fp.practice_school_dw_id
         JOIN coredw.dim_grade dg ON fp.practice_grade_dw_id = dg.grade_dw_id AND dg.grade_status <> 4
         JOIN bi_coredw.bi_student_dim_mv dst
              ON dst.student_dw_id = fp.practice_student_dw_id AND dst.student_status <> 4
         JOIN coredw.dim_tenant dte ON dte.tenant_dw_id = fp.practice_tenant_dw_id
         JOIN coredw.dim_date dd ON dd.date_id = to_char(
    ...
    AND full_date between trunc(sysdate) - 365 and trunc(sysdate)
         JOIN coredw.dim_learning_objective lo ON fp.practice_lo_dw_id = lo.lo_dw_id
         LEFT JOIN coredw.dim_academic_year ay ON ay.academic_year_dw_id = fp.practice_academic_year_dw_id
         LEFT JOIN coredw.dim_section dc
                   ON dc.section_dw_id = fp.practice_section_dw_id AND dc.section_status <> 4
         LEFT JOIN coredw.dim_class dcs ON fp.practice_class_dw_id = dcs.class_dw_id AND dcs.class_status = 1
         LEFT JOIN coredw.dim_subject dsb ON fp.practice_subject_dw_id = dsb.subject_dw_id
         LEFT JOIN (SELECT dda.date_id,
                    ...
                    FROM coredw.fact_practice_session fps
                             JOIN coredw.dim_tenant dten ON dten.tenant_dw_id = fps.practice_session_tenant_dw_id
                             JOIN coredw.dim_date dda ON dda.date_id = to_char(
