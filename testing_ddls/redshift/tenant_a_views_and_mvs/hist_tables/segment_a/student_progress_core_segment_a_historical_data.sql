DROP TABLE IF EXISTS bi_coredw_dev.student_progress_core_military_historical_data;
CREATE TABLE bi_coredw_dev.student_progress_core_military_historical_data
    SORTKEY
(
    local_date,
    student_dw_id
...
WITH fact AS (
    SELECT local_date,
           ...
           fle_session_time,
           fle_score
    FROM bi_coredw.students_lesson_progress_military_historical_data
    UNION ALL
        SELECT local_date,
           student_dw_id,
           lo_attempted,
...
FROM coredw.dim_class_user dcu
         JOIN bi_coredw_dev.core_class_activity_content_mv cont
             ON dcu.class_user_class_dw_id = cont.class_dw_id
         JOIN coredw.dim_student ds
