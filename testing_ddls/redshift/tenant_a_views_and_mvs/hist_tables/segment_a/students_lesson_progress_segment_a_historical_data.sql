DROP TABLE IF EXISTS bi_coredw_dev.students_lesson_progress_military_historical_data;
CREATE TABLE bi_coredw_dev.students_lesson_progress_military_historical_data DISTKEY (school_dw_id)
...
WITH COMPLETED_LESSONS AS
         (SELECT *
          FROM (SELECT fle_ls_id,
                ...
                FROM (
                    SELECT
                        fle_ls_id, fle_dw_id, fle_total_score, fle_score, fle_created_time, fle_lo_dw_id, fle_school_dw_id FROM coredw.fact_learning_experience
                        where
     ...
     LESSON_PROGRESS AS
         (SELECT fle_dw_id, 0 AS fle_score, 'In-Progress' AS lo_status
          FROM (SELECT fle_ls_id,
                ...
                FROM coredw.fact_learning_experience flee
                         JOIN bi_coredw.bi_all_schools_dim_mv dsc
                              ON fle_school_dw_id = dsc.school_dw_id
                                  AND trunc(fle_created_time) >= dsc.academic_year_start_date
                                  AND trunc(fle_created_time) <= dsc.academic_year_end_date
    ...
    FROM coredw.dim_class_user dcu
    JOIN bi_coredw.core_class_activity_content_mv cac
...
FROM (SELECT DISTINCT
      ...
      FROM coredw.fact_learning_experience fle
               JOIN coredw.dim_academic_year ay ON fle.fle_academic_year_dw_id = ay.academic_year_dw_id
               ...
               JOIN (
                ...
                FROM
                    bi_coredw.bi_student_dim_mv bs
                JOIN
                    coredw.dim_grade
                    ON student_grade_dw_id = grade_dw_id
                JOIN
                    bi_coredw.bi_all_schools_dim_mv
                ...
                WHERE
                    ...
                    AND dst.rnk = 1
               JOIN coredw.dim_grade dg ON dg.grade_dw_id = fle.fle_grade_dw_id
      ...
      WHERE fle_abbreviation <> 'NA'
        ...
        AND extract(YEAR FROM dsc.academic_year_end_date) < (
            ...
            FROM bi_coredw.bi_all_schools_dim_mv asd
