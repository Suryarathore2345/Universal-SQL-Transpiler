DROP TABLE IF EXISTS bi_coredw_dev.pathway_activity_historical_data;
CREATE TABLE bi_coredw_dev.pathway_activity_historical_data AS
WITH fact_levels_recommended_last AS (SELECT fpta_created_time                    AS flr_created_time,
                                      ...
                                      FROM (SELECT fpta_created_time,
                                            ...
                                            FROM coredw.fact_pathway_teacher_activity) fpta
                                      WHERE fpta.rank = 1
                                      ...
                                      FROM (SELECT flr_created_time,
                                            ...
                                            FROM coredw.fact_levels_recommended flr
                                                     INNER JOIN coredw.dim_course_activity_association dccaa
...
--                                                                     AND dccaa.caa_status = 1
     ...
     fact_pathway_activity_completed_last AS ( -- to take latest attempt of completed activity
         ...
         FROM (SELECT fpac.fpac_learning_session_id,
               ...
               FROM coredw.fact_pathway_activity_completed fpac
                        INNER JOIN coredw.dim_course_activity_association dccaa
                                   ON dccaa.caa_activity_dw_id = fpac.fpac_activity_dw_id
                                       AND dccaa.caa_status = 1) fpacl
         WHERE rank = 1),
     ...
     fact_learning_experience_last AS (SELECT *
                                       FROM (SELECT fle_lesson_category,
                                             ...
                                             FROM coredw.fact_learning_experience fle
     ...
     pathway_adt_students as (SELECT DISTINCT fle.fle_student_dw_id             AS adt_student_dw_id,
                              ...
                              FROM coredw.fact_learning_experience fle
                                       INNER JOIN bi_coredw.bi_student_dim_mv ds
                                                  ON ds.student_dw_id = fle.fle_student_dw_id
                                       INNER JOIN coredw.dim_class dc
                                                  ON fle.fle_class_dw_id = dc.class_dw_id
                                       LEFT JOIN coredw.dim_course_subject_association dcsa
                                                 ON FNV_HASH(dcsa.cs_course_id) = FNV_HASH(dc.class_material_id)
                              ...
                              WHERE fle_lesson_category = 'DIAGNOSTIC_TEST'
                                ...
                                AND fle_exp_id = 'n/a'),
     prev_ay AS (SELECT school_dw_id,
                        school_name,
                        tenant_id,
                        tenant_name,
                        organisation_dw_id,
                        ...
                        academic_year_id
                 FROM bi_coredw.bi_all_schools_dim_mv
                 WHERE academic_year_is_roll_over_completed
                   AND date_part_year(academic_year_end_date) >=
                       (SELECT max(date_part_year(academic_year_end_date)) - 2
                        FROM bi_coredw.bi_all_schools_dim_mv) --selecting current and previous 2 academic years
     ),
     pathway_class_total_students AS (SELECT dsc.school_dw_id,
                                      ...
                                      FROM coredw.dim_class dc
                                               INNER JOIN coredw.dim_class_user dcu
                                                          ON dcu.class_user_class_dw_id = dc.class_dw_id
                                               INNER JOIN prev_ay dsc
                                                          ON FNV_HASH(dc.class_school_id) = FNV_HASH(dsc.school_id)
                                                              AND FNV_HASH(dsc.academic_year_id) =
                                               ...
                                               INNER JOIN coredw.dim_grade dg
                                                          ON dg.grade_dw_id = ds.student_grade_dw_id
                                                              AND FNV_HASH(dsc.academic_year_id) =
                                                                  FNV_HASH(dg.academic_year_id)
                                                              AND grade_status = 1
                                               LEFT JOIN coredw.dim_course_subject_association dcsa
                                                         ON FNV_HASH(dcsa.cs_course_id) = FNV_HASH(dc.class_material_id)
                                                             AND dcsa.cs_subject_dw_id = 129
...
SELECT DISTINCT pcts.tenant_id,
                ...
                CASE
                    WHEN flr.flr_recommendation_type = -1 THEN 'MANUALLY'
                    WHEN (flr.flr_recommendation_type < 5 AND curr_subject_name <> 'Science') THEN 'BY DIAGNOSTIC TEST'
                    WHEN (flr.flr_recommendation_type < 5 AND curr_subject_name = 'Science') THEN 'AT GRADE LEVEL'
                    WHEN flr.flr_recommendation_type = 5 THEN 'AT GRADE LEVEL'
                    WHEN flr.flr_recommendation_type = 6 THEN 'AT BEGINNING OF COURSE'
...
FROM pathway_class_total_students pcts
         LEFT JOIN coredw.dim_course dcr
         ...
         LEFT JOIN fact_levels_recommended_last flr
                   ON pcts.pathway_student_dw_id = flr.flr_student_dw_id
                       ...
                       AND flr.flr_created_time BETWEEN
                          pcts.academic_year_start_date AND pcts.academic_year_end_date
         LEFT JOIN coredw.dim_course_activity_container dcac
                   ON FNV_HASH(dcac.course_activity_container_course_id) = FNV_HASH(dcr.course_id)
                       AND dcac.course_activity_container_dw_id = flr.flr_course_activity_container_dw_id
                       AND dcac.course_activity_container_created_time BETWEEN
                          pcts.academic_year_start_date AND pcts.academic_year_end_date
                       AND dcac.course_activity_container_status = 1
                       AND dcac.course_activity_container_attach_status = 1
         LEFT JOIN coredw.fact_level_completed flc
                   ON flc.flc_course_activity_container_dw_id = flr.flr_course_activity_container_dw_id
                       ...
                       AND flc.flc_created_time BETWEEN
                          pcts.academic_year_start_date AND pcts.academic_year_end_date
         ...
         LEFT JOIN coredw.fact_lesson_feedback flf
                   ON flf.lesson_feedback_lo_dw_id = fpac.fpac_activity_dw_id
                       ...
                       AND flf.lesson_feedback_created_time BETWEEN
                          pcts.academic_year_start_date AND pcts.academic_year_end_date
         LEFT JOIN pathway_adt_students pnadt
                   ON pcts.pathway_student_dw_id = pnadt.adt_student_dw_id
                       ...
                       AND pnadt.adt_created_time BETWEEN pcts.academic_year_start_date AND pcts.academic_year_end_date;