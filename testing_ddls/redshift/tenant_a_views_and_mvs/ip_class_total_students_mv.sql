CREATE materialized view bi_coredw_dev.ip_class_total_students_mv
AS
WITH CLASS_TEACHERS AS (
    ...
    from coredw.dim_class dc
             ...
             LEFT JOIN coredw.dim_teacher dt ON dcu.class_user_user_dw_id = dt.teacher_dw_id and dt.teacher_status = 1
               and teacher_id NOT IN (SELECT DISTINCT teacher_id FROM bi_coredw.exclude_teacher_id)
...
FROM coredw.dim_class dc
         ...
         JOIN bi_coredw.bi_active_schools_dim_mv sc ON md5(dc.class_school_id) = md5(sc.school_id)
         JOIN coredw.dim_student ds ON dcu.class_user_user_dw_id = ds.student_dw_id
                                    AND sc.school_dw_id = ds.student_school_dw_id
         JOIN coredw.dim_content_academic_year dcay ON md5(dc.class_content_academic_year) = md5(dcay.content_academic_year_name)
         JOIN coredw.dim_curriculum_grade dcg
              ON md5(dc.class_curriculum_grade_id) = md5(dcg.curr_grade_id)
         JOIN coredw.dim_curriculum_subject dcs
         ...
         JOIN coredw.dim_grade dg on md5(dg.grade_id) = md5(dc.class_grade_id)
