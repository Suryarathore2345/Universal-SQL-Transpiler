CREATE OR REPLACE VIEW bi_coredw_dev.teacher_score_idn_report_view AS
(
WITH holidays_dimension AS (SELECT DISTINCT cast(holiday_date AS DATE) AS holiday_date
     ...
     class_teachers AS (SELECT dc.class_dw_id
             ...
             ORDER BY class_user_created_time
                        ...
                        FROM coredw.dim_class dc
                                 ...
                                 LEFT JOIN coredw.dim_teacher dt ON dcu.class_user_user_dw_id = dt.teacher_dw_id
     ...
     active_teachers AS (SELECT temp_tab.*,
                         ...
                         FROM (SELECT DISTINCT trunc(login_local_date_time)                AS login_date
                               ...
                               FROM bi_coredw.teacher_login tl
                                        ...
                                        INNER JOIN coredw.dim_teacher dt ON dt.teacher_dw_id = tl.teacher_dw_id
                               ...
                               WHERE trunc(tl.login_local_date_time) BETWEEN dsc.academic_year_start_date
                                 ...
                                 AND dt.teacher_id NOT IN
                                     (SELECT DISTINCT teacher_id FROM bi_coredw.exclude_teacher_id)) temp_tab),
     ...
     active_students AS (select distinct date_trunc('month',
                         ...
                         FROM coredw.fact_student_activities fsta
                                  JOIN coredw.dim_tenant dt
                                       on fsta.fsta_tenant_dw_id = dt.tenant_dw_id
                                  LEFT JOIN coredw.dim_class_user dcu
                                            ON dcu.class_user_user_dw_id = fsta.fsta_student_dw_id
                                  LEFT JOIN coredw.dim_class dc on dcu.class_user_class_dw_id = dc.class_dw_id
                         WHERE split_part(fsta_object_id, '/', 5)
                             IN (select distinct lo_id
                                 from coredw.dim_learning_objective)
                           AND tenant_name = 'idn'
     ...
     total_in_curriculum AS
         (SELECT month_start_date,
          ...
          FROM coredw.dim_class dc
                   ...
                   LEFT JOIN bi_coredw.bi_student_dim_mv ds
                             ON dcu.class_user_user_dw_id = ds.student_dw_id
                                 AND sc.school_dw_id = ds.student_school_dw_id
                   JOIN coredw.dim_grade dg on dg.grade_dw_id = ds.student_grade_dw_id
                   LEFT JOIN coredw.dim_section dse
