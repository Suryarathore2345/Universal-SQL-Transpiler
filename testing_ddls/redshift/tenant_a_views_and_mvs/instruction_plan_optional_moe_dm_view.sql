CREATE OR REPLACE VIEW bi_coredw_dev.instruction_plan_optional_moe_dm_view
AS

WITH Learning_Objective as (SELECT lo_dw_id, lo_code, lo_title, lo_created_time
                            FROM coredw.dim_learning_objective dip_dlo
     ...
     Civil_lo AS (SELECT DISTINCT caa_course_id,
                  ...
                  FROM coredw.dim_course_activity_association
     ...
     class_total_students_civil_defense as (SELECT dc.class_dw_id,
                                            ...
                                            FROM coredw.dim_class dc
                                                     ...
                                                     INNER JOIN academic_year lay
                                                                ON ac.school_dw_id = lay.school_dw_id
                                                     LEFT JOIN coredw.dim_curriculum_grade dcg
                                                               ON dc.class_curriculum_grade_id =
                                                                  dcg.curr_grade_id
                                                     LEFT JOIN coredw.dim_grade dg
                                                               ON md5(dg.grade_id) = md5(dc.class_grade_id)
                                                     LEFT JOIN coredw.dim_section dsec
     ...
     students_learning_progress_civil_defense as (SELECT fl.*,
                                                  ...
                                                  FROM (SELECT DISTINCT dd.full_date               AS local_date,
                                                        ...
                                                        FROM coredw.fact_learning_experience fle
                                                                 ...
                                                                 INNER JOIN coredw.dim_student ds
...
FROM class_total_students_civil_defense cts
         ...
         JOIN coredw.dim_academic_calendar dac
         ...
         JOIN coredw.dim_academic_calendar_teaching_period dtrm
         ...
         LEFT JOIN coredw.dim_student ds
                   ON ds.student_dw_id = lp.student_dw_id
         LEFT JOIN coredw.dim_date dd
