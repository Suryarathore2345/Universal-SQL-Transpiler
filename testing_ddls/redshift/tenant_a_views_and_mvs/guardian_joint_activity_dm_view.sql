...
CREATE OR REPLACE VIEW bi_coredw_dev.guardian_joint_activity_dm_view AS
WITH fact_guardian_joint_activity AS (SELECT DISTINCT fgja.*,
                                      ...
                                      FROM (SELECT fgja.*,
                                            ...
                                            FROM (SELECT *,
                                                  ...
                                                  FROM coredw.fact_guardian_joint_activity) AS fgja
                                            ...
                                            WHERE fgja.ROW = 1) as fgja
                                               ...
                                               JOIN coredw.dim_course_activity_container dcac
...
FROM fact_guardian_joint_activity as fgja
         JOIN bi_coredw.bi_student_dim_mv as dst
              ON dst.student_dw_id = fgja.fgja_student_dw_id
                  AND dst.student_status = 1
         JOIN bi_coredw.bi_active_schools_dim_mv as dsc
         ...
         JOIN (SELECT DISTINCT school_dw_id,
               ...
               FROM bi_coredw.guardian_student_association_dm_view as gsa
               GROUP BY 1) AS gsa
