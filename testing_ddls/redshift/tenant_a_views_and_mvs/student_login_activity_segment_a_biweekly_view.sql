CREATE OR REPLACE VIEW bi_coredw_dev.student_login_activity_military_biweekly_view AS
WITH total_students AS -- Section Level
     ...
     active_students AS -- Section Level
         (SELECT DISTINCT login_date,
          ...
          FROM (SELECT DISTINCT Trunc(login_local_date_time) AS login_date,
                ...
                FROM bi_coredw.student_login_military sl
                         INNER JOIN bi_coredw.bi_active_schools_dim_mv dsc
                ...
                WHERE trunc(login_local_date_time) >= trunc(convert_timezone('UTC', dsc.tenant_timezone, sysdate)) - 14
     ...
     student_onboarding as (select distinct student_dw_id,
                            ...
                            from bi_coredw.student_login_military sl
                                     INNER JOIN bi_coredw.bi_active_schools_dim_mv ds
                                                ...
                                                AND trunc(login_local_date_time) >= academic_year_start_date
),
     ...
     student_onboarding_pay as
        ...
        and trunc(sl.login_local_date_time) between spay.previous_academic_year_start_date and spay.previous_academic_year_end_date
        ),
...
WHERE ts.school_organisation = 'MHS'
