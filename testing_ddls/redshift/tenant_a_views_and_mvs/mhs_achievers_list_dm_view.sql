CREATE OR REPLACE VIEW bi_coredw_dev.mhs_achievers_list_dm_view AS
WITH cte_term_dimension AS
    ...
    FROM bi_coredw.bi_active_schools_dim_mv sch
        LEFT JOIN coredw.dim_academic_calendar dac
    ...
    WHERE organisation_name = 'MHS'),
     ...
     FROM coredw.fact_guardian_app_activities ga
    ...
    cte_instruction_plan AS (
     ...
     FROM bi_coredw.student_progress_ip_mscl_view ip
    ...
    FROM(
        ...
        FROM coredw.fact_learning_experience fle1
        INNER JOIN (SELECT DISTINCT class_dw_id, class_gen_subject, class_school_id FROM coredw.dim_class) dc
    ...
    cte_certificates_awarded AS (
         ...
         FROM bi_coredw.student_certificate_awarded_dm_view cert
          INNER JOIN (SELECT DISTINCT class_dw_id, class_gen_subject, class_school_id FROM coredw.dim_class) dc
