CREATE OR REPLACE view bi_coredw_dev.ktgames_dm_view AS
WITH recommended_games as
         (select *
          from (SELECT DISTINCT ktg_id,
                ...
                FROM coredw.fact_ktg fkg
                         JOIN bi_coredw.bi_active_schools_dim_mv dsc ON dsc.school_dw_id = fkg.ktg_school_dw_id
                    AND trunc(ktg_created_time) >= dsc.academic_year_start_date
                    AND trunc(ktg_created_time) <= dsc.academic_year_end_date
                         LEFT JOIN coredw.dim_subject dsu ON dsu.subject_dw_id = fkg.ktg_subject_dw_id
                         JOIN coredw.dim_grade dg ON dg.grade_dw_id = fkg.ktg_grade_dw_id
                         LEFT JOIN coredw.dim_section dse ON dse.section_dw_id = fkg.ktg_section_dw_id
                         LEFT JOIN coredw.dim_class dc ON dc.class_dw_id = fkg.ktg_class_dw_id
     ...
     ktg_sessions_dataset as
         (SELECT distinct ktg_session_id
          ...
          FROM coredw.fact_ktg_session fks
