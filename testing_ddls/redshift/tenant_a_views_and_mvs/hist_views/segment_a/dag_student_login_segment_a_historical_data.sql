CREATE OR REPLACE VIEW bi_coredw_dev.dag_student_login_military_historical_data AS
...
WITH calendar_year AS (SELECT DISTINCT full_date
     ...
     login_data AS (SELECT DISTINCT sl.login_local_date_time::DATE AS local_date,
                    ...
                    FROM bi_coredw.student_login sl
                             INNER JOIN coredw.dim_tenant t ON t.tenant_dw_id = sl.tenant_dw_id
                             INNER JOIN
                         bi_coredw.bi_student_dim_mv ds
                         ON ds.student_dw_id = sl.student_dw_id
                             INNER JOIN coredw.dim_grade dg
                                        ON dg.grade_dw_id = ds.student_grade_dw_id
                             INNER JOIN coredw.dim_school sch
                    ...
                    WHERE
                        sch.school_organization_dw_id = 29
...
FROM (SELECT DISTINCT date_part_year(ay.academic_year_start_date)::VARCHAR + ' - ' +
                      ...
                      grade_dw_id,
                      grade_name
      FROM coredw.dim_student ds
               CROSS JOIN calendar_year cy
               inner join coredw.dim_grade g ON ds.student_grade_dw_id = g.grade_dw_id
               inner join coredw.dim_school sch
               ...
               inner join coredw.dim_tenant t ON t.tenant_id = sch.school_tenant_id
      WHERE sch.school_organization_dw_id = 29
        AND (
        ...
        AND (full_date between ay.academic_year_start_date AND ay.academic_year_end_date)) reg_data
