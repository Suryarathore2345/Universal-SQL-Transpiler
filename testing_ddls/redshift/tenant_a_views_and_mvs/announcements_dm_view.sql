CREATE OR REPLACE VIEW bi_coredw_dev.announcements_dm_view AS
...
FROM coredw.fact_announcement a
         INNER JOIN coredw.dim_role r ON r.role_dw_id = a.fa_role_dw_id
         INNER JOIN coredw.dim_staff_user dp ON dp.staff_user_dw_id = a.fa_admin_dw_id
         INNER JOIN coredw.dim_staff_user_school_role_association dsusra
                    on dsusra.susra_staff_dw_id = dp.staff_user_dw_id
         INNER JOIN bi_coredw.bi_active_schools_dim_mv sch ON sch.school_dw_id = dsusra.susra_school_dw_id
...
FROM coredw.fact_announcement a
         ...
         INNER JOIN bi_coredw.bi_active_schools_dim_mv sch ON sch.school_dw_id = tch.teacher_school_dw_id
