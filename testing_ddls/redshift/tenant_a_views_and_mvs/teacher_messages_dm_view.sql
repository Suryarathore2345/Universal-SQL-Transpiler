CREATE OR REPLACE VIEW bi_coredw_dev.teacher_messages_dm_view AS
WITH responses AS ( -- define responded messages - teacher messages with at least 1 guardian message in the thread
    SELECT DISTINCT thread.tft_thread_id,
                    ...
                    true AS response
    FROM coredw.dim_teacher_feedback_thread thread
             JOIN (SELECT DISTINCT tft_thread_id,
                                   tft_message_id AS teacher_message_id
                   FROM coredw.dim_teacher_feedback_thread
...
FROM coredw.dim_teacher_feedback_thread m
...
FROM messages m
         JOIN bi_coredw.bi_student_dim_mv st ON st.student_dw_id = m.tft_student_dw_id
    AND student_status = 1
         JOIN bi_coredw.bi_active_schools_dim_mv sch ON sch.school_dw_id = st.student_school_dw_id
GROUP BY 1, 2, 3
