CREATE OR REPLACE VIEW bi_coredw_dev.stars_awarded_dm_view AS
...
FROM coredw.fact_star_awarded fsa
         ...
         JOIN coredw.dim_grade dg ON dg.grade_dw_id = fsa.fsa_grade_dw_id
    AND dg.grade_status <> 4
         JOIN bi_coredw.bi_student_dim_mv ds ON ds.student_dw_id = fsa.fsa_student_dw_id
    AND ds.student_status <> 4
         JOIN coredw.dim_award_category ac ON ac.award_category_dw_id = fsa.fsa_award_category_dw_id
         LEFT JOIN coredw.dim_class dc ON dc.class_dw_id = fsa.fsa_class_dw_id
         LEFT JOIN coredw.dim_subject dsu ON dsu.subject_dw_id = fsa.fsa_subject_dw_id
