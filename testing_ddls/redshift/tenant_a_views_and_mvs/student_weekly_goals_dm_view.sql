CREATE OR REPLACE VIEW bi_coredw_dev.student_weekly_goals_dm_view AS
...
--                             , dcr.curr_name
             ...
             END                                                          AS goal_status,
                       ...
                       FROM coredw.fact_weekly_goal fwg
                                INNER JOIN coredw.dim_weekly_goal_type dwgt
                                           on dwgt.weekly_goal_type_dw_id = fwg.fwg_type_dw_id
                                INNER JOIN bi_coredw.bi_student_dim_mv sdm
                                ...
                                INNER JOIN coredw.dim_grade dg on dg.grade_dw_id = sdm.student_grade_dw_id
                           and MD5(dsc.academic_year_id) = MD5(dg.academic_year_id)
                                INNER JOIN coredw.dim_class dc on DC.class_dw_id = fwg.fwg_class_dw_id
                           AND MD5(dsc.academic_year_id) = MD5(dc.class_academic_year_id)
                           AND class_status = 1
                           AND class_course_status = 'ACTIVE'
                                INNER JOIN coredw.dim_section dse on dse.section_dw_id = sdm.student_section_dw_id
...
--                                 INNER JOIN coredw.dim_curriculum dcr
