CREATE MATERIALIZED VIEW bi_coredw_dev.core_class_ic_content_mv AS
WITH ip_base AS (
    SELECT course_dw_id,
           course_id,
           activity_dw_id,
           ...
           activity_is_hidden,
           event_type
    FROM coredw.dim_course_activity_ip_association
...
ip_ranked AS (
    ...
    FROM ip_base ip2
    JOIN coredw.dim_class dc2 ON ip2.course_id = dc2.class_material_id AND dc2.class_status = 1
    JOIN coredw.dim_grade g ON g.grade_id = dc2.class_grade_id
    JOIN bi_coredw.bi_all_schools_dim_mv sch2
...
UNION ALL
SELECT * FROM bi_coredw.core_ip_class_ic_content_mv;