CREATE OR REPLACE VIEW bi_coredw_dev.core_course_ic_classlearning_agg_view AS
WITH class_total_students AS (
SELECT COALESCE(cts.class_dw_id, cts_prev.class_dw_id)                   AS class_dw_id,
...
FULL OUTER JOIN bi_coredw.class_total_students_prev_ay_mv cts_prev
...
core_course_ic_experience AS (
    SELECT fle_class_dw_id,
           ...
           COUNT(DISTINCT CASE WHEN ic_status = 'Completed' THEN student_dw_id END)  AS total_completed_students,
           COUNT(DISTINCT CASE WHEN fle_total_score >= 70 THEN student_dw_id END) AS meets_completed_students,
           COUNT(DISTINCT CASE WHEN fle_total_score >= 50 AND fle_total_score < 70 THEN student_dw_id END) AS approaching_completed_students,
           COUNT(DISTINCT CASE WHEN fle_total_score < 50 AND fle_total_score >= 0 THEN student_dw_id END)  AS below_completed_students,
           SUM(fle_total_score)  AS fle_score,
           SUM(session_time)                                                             AS session_time
    FROM bi_coredw.students_ic_progress_mv
    GROUP BY 1,2,3
    UNION ALL
    SELECT *
    FROM bi_coredw.core_course_ic_experience_mv ccfle
    WHERE NOT EXISTS(SELECT 1 FROM bi_coredw.students_ic_progress_mv slp WHERE ccfle.fle_class_dw_id = slp.fle_class_dw_id)
)
SELECT cont.*,
       cts.section_dw_id,
       cts.section_name,
       ...
       fact.session_time
FROM bi_coredw.core_class_ic_content_mv cont
