CREATE OR REPLACE VIEW bi_coredw_dev.incgames_dm_view AS
...
SELECT
        ...
        FROM bi_coredw.class_total_students_mv st join bi_coredw.bi_active_schools_dim_mv ach
          ...
          SELECT
                ...
                st.class_title,
                st.class_total_students
          from bi_coredw.ip_class_total_students_mv st join bi_coredw.bi_active_schools_dim_mv ach
              on st.school_dw_id = ach.school_dw_id
)
SELECT DISTINCT
                f.inc_game_id,
...
FROM coredw.fact_inc_game f -- capture a game
    INNER JOIN
    (
           select
           ...
           from coredw.fact_inc_game_session
                --where inc_game_session_status NOT IN (1, 4)
          ) fs ON md5(fs.inc_game_session_game_id) = md5(f.inc_game_id)
         LEFT JOIN coredw.fact_inc_game_outcome fo ON md5(fo.inc_game_outcome_game_id) = md5(f.inc_game_id)
               and fs.inc_game_session_id = fo.inc_game_outcome_session_id-- outcome of the game
         INNER JOIN coredw.dim_tenant dt ON dt.tenant_dw_id = f.inc_game_tenant_dw_id
         INNER JOIN coredw.dim_grade dg ON dg.grade_dw_id = f.inc_game_grade_dw_id
                AND dg.grade_status = 1
         INNER JOIN coredw.dim_learning_objective lo ON lo.lo_dw_id = f.inc_game_lo_dw_id
                AND lo.lo_status = 1
             INNER JOIN bi_coredw.bi_active_schools_dim_mv ds ON ds.school_dw_id = f.inc_game_school_dw_id
