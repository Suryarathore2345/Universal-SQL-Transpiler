CREATE MATERIALIZED VIEW bi_coredw_dev.magg_student_kpi_historical_mv AUTO REFRESH YES AS
WITH reg_data AS (
    SELECT
    ...
    FROM bi_coredw.students_stats_monthly
    GROUP BY
        calendar_year_end_date,
        calendar_month_end_date,
        calendar_month_start_date,
...
max_insert_date AS (
    SELECT MAX(inserted_at) AS inserted_at  FROM bi_coredw.students_stats_monthly
                              )
