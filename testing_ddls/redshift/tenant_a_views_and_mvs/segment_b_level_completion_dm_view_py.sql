CREATE OR REPLACE VIEW bi_coredw_dev.pathway_level_completion_dm_view_py AS
WITH school_prveviousay AS (                   -- define previous Academic Year start and end date by school
			...
			FROM (
				...
				FROM bi_coredw.bi_all_schools_dim_mv ay
    ...
    FROM (
             ...
             FROM coredw.fact_levels_recommended flc
                      INNER JOIN coredw.dim_course_activity_association dplaa
...
INNER JOIN coredw.dim_school sch ON md5(flr.class_school_id)  = md5(sch.school_id)
