CREATE OR REPLACE VIEW bi_coredw_dev.product_level_kpis_monthly_view AS
         ...
         (SELECT month_start_date,
                 ...
                 CASE
                    ...
                    WHEN csa.cs_subject_dw_id is null AND dc.class_material_type = 'CORE'
                        OR (dc.class_material_type = 'INSTRUCTIONAL_PLAN' AND dc.class_curriculum_subject_id != 963534) THEN 'CORE'
         ...
         (SELECT month_start_date,
          ...
          FROM coredw.dim_class dc
                   ...
                   JOIN bi_coredw.bi_active_schools_dim_mv sc
                        ON md5(dc.class_school_id) = md5(sc.school_id)
                            AND dc.class_academic_year_id=sc.academic_year_id
                   LEFT JOIN coredw.dim_teacher t
