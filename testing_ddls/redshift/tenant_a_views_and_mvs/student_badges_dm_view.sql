CREATE OR REPLACE VIEW bi_coredw_dev.student_badges_dm_view
AS
WITH section_total_students AS ( -- to take section total students
    ...
    FROM coredw.dim_section dse
             JOIN bi_coredw.bi_active_schools_dim_mv sc ON md5(dse.school_id) = md5(sc.school_id)
             ...
             JOIN coredw.dim_grade dg ON md5(dg.grade_dw_id) = md5(ds.student_grade_dw_id)
    ...
    GROUP BY 1, 2, 3, 4, 5, 6
     ...
     earned_badges AS (
                    SELECT dd.full_date AS local_date,
                      ...
                      fba_date_dw_id
               FROM coredw.fact_badge_awarded fba
                        JOIN coredw.dim_date dd
...
FROM earned_badges eb
         INNER JOIN coredw.dim_badge db ON db.bdg_dw_id = eb.fba_badge_dw_id
    AND db.bdg_status IN (1,2)
         INNER JOIN bi_coredw.bi_student_dim_mv dst
