CREATE OR REPLACE VIEW bi_coredw_dev.pathway_activity_dm_view as
...
UNION ALL
    ...
    WHERE flrdplaa.rank = 1
     ...
     fact_pathway_activity_completed_last AS ( -- to take latest attempt of completed activity
         ...
         FROM (SELECT fpac.*,
                      ROW_NUMBER() OVER (
                          PARTITION BY fpac_learning_session_id ORDER BY fpac_attempt DESC, fpac_created_time DESC
                          ) AS rank
