CREATE OR REPLACE VIEW bi_coredw_dev.core_course_classlearning_agg_view AS
WITH class_total_students AS (
    ...
    FROM
        bi_coredw.class_total_students_mv cts
    FULL OUTER JOIN bi_coredw.class_total_students_prev_ay_mv cts_prev ON cts.class_dw_id = cts_prev.class_dw_id
),
core_course_learning_experience AS (
    ...
    UNION ALL
    SELECT *
    FROM bi_coredw.core_course_learning_experience_mv ccfle
    WHERE NOT EXISTS(SELECT 1 FROM bi_coredw.students_lesson_progress_mv slp WHERE ccfle.fle_class_dw_id = slp.fle_class_dw_id)
)
SELECT cont.*,
       cts.section_dw_id,
       cts.section_name,
       ...
       fact.session_time
FROM bi_coredw.core_class_activity_content_mv cont
