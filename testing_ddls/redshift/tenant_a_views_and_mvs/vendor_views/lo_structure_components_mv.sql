CREATE MATERIALIZED VIEW bi_coredw_dev.lo_structure_components_mv AS
(
...
--                dat.at_activity_type                                                  AS component_type, -- when needed we can uncomment this
                ...
                dasa.status
FROM coredw.dim_activity_section_association AS dasa
         JOIN coredw.dim_learning_objective dlo
              ON dasa.activity_id = dlo.lo_id
         JOIN coredw.dim_content_section dcs ON dcs.id = dasa.section_id
         JOIN coredw.dim_content_slide dsl ON dsl.section_id = dcs.id
         JOIN coredw.dim_tenant dt ON dt.tenant_id = dasa.tenant_id
--         JOIN coredw.dim_activity_template dat ON dat.at_component_uuid = dasa.template_component_uuid    -- when needed we can uncomment this
