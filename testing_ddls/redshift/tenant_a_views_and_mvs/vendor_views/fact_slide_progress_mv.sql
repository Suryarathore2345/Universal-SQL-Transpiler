CREATE MATERIALIZED VIEW bi_coredw_dev.fact_slide_progress_mv
AS
(
WITH class_total AS (SELECT DISTINCT dc.class_dw_id,
                     ...
                     FROM coredw.dim_class dc
                              ...
                              JOIN bi_coredw.bi_student_dim_mv ds ON ds.student_dw_id = dcu.class_user_user_dw_id
   ...
   , FACT_SLIDE_COMPLETED AS
    ...
    SELECT DISTINCT TRUNC(CONVERT_TIMEZONE('UTC'
     ...
     FROM coredw.fact_student_slide_progress fssp
         ...
         AND ct.school_id = dsc.school_id
              ...
              JOIN coredw.dim_class_user dcu ON dcu.class_user_class_dw_id = fssp.class_dw_id
              JOIN coredw.dim_class dc on dc.class_dw_id = fssp.class_dw_id
         AND dcu.class_user_user_dw_id = fssp.student_dw_id
              JOIN coredw.dim_grade dg ON dg.grade_id = fssp.grade_id
