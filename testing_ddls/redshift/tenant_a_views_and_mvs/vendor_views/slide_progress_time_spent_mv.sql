...
CREATE MATERIALIZED VIEW bi_coredw_dev.slide_progress_time_spent_mv
AS (
    WITH daily_time_spent_per_slide AS (SELECT    sl.local_date             AS local_date,
                                           ...
                                           FROM bi_coredw.fact_slide_progress_mv sl
--                                            where rnk = 1
                                           ),
           ...
           slides_completed_per_stud_per_lo as (select MAX(fsl.local_date)          as local_date,
                                                ...
                                                from bi_coredw.fact_slide_progress_mv fsl
                                                         join coredw.dim_content_slide dcl
           ...
           slides_assigned_per_lo as (select activity_dw_id,
                                      ...
                                      from bi_coredw.lo_structure_components_mv
