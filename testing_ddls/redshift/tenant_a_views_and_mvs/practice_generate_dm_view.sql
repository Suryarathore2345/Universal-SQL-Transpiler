CREATE OR REPLACE VIEW bi_coredw_dev.practice_generate_dm_view AS
...
FROM coredw.dim_learning_objective lo
	INNER JOIN coredw.dim_step_instance dsi
	    ON lo.lo_id=dsi.step_instance_lo_id
	INNER JOIN coredw.dim_content_association c
	    ON dsi.step_instance_id=c.content_association_content_id
	INNER JOIN coredw.dim_ccl_skill sk
		ON c.content_association_id = sk.ccl_skill_id
	INNER JOIN coredw.dim_skill_association ska
 ...
 lesson_wth_practice2  AS (-- lessons with practice prerequisite missing in lesson_wth_practice1
     SELECT DISTINCT lo.lo_dw_id,
                     ...
                     lo.lo_curriculum_subject_id
     FROM coredw.dim_learning_objective lo
     INNER JOIN coredw.fact_practice p
...
INNER JOIN coredw.dim_curriculum_subject sj
	ON lwp.lo_curriculum_subject_id = sj.curr_subject_id
INNER JOIN coredw.dim_grade gr
	ON gr.grade_dw_id = st_et.fle_grade_dw_id
INNER JOIN coredw.dim_instructional_plan d_ip
