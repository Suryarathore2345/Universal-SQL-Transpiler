CREATE MATERIALIZED VIEW bi_coredw_dev.student_login_activity_mv AS
WITH total_students AS -- Section Level
         (SELECT DISTINCT full_date                                                      AS local_date,
                          ...
                          school_created_time,
                          cast(extract(year from dsc.academic_year_start_date) as varchar) || '-' ||
                          cast(extract(year from dsc.academic_year_end_date) as varchar) as academic_year,
          ...
          FROM (SELECT full_date, section_name, section_dw_id, grade_id, school_id, tenant_id, section_id
                FROM coredw.dim_section
                         CROSS JOIN (SELECT distinct full_date
                                     FROM coredw.dim_date dt
                                     WHERE dt.full_date between trunc(sysdate) - 90 and trunc(sysdate))
                WHERE school_id is not null) dse
                   INNER JOIN bi_coredw.bi_active_schools_dim_mv dsc
          ...
          where dsc.school_id = '3e9bf712-c13e-43e1-a64f-d5180ea9ee9c' -- keeps only sports academy school
         ),
     ...
     active_students AS -- Section Level
         (SELECT DISTINCT login_date,
          ...
          FROM (SELECT DISTINCT Trunc(login_local_date_time) AS login_date,
                ...
                FROM bi_coredw.student_login sl
                         INNER JOIN bi_coredw.bi_active_schools_dim_mv dsc
                ...
                WHERE dsc.school_id = '3e9bf712-c13e-43e1-a64f-d5180ea9ee9c' -- keeps only sports academy school
                  AND  trunc(login_local_date_time) >=
                      trunc(convert_timezone('UTC', dsc.tenant_timezone, sysdate)) - 90
     ...
     student_onboarding as (select distinct student_dw_id,
                            ...
                            from bi_coredw.student_login sl
                                     INNER JOIN bi_coredw.bi_active_schools_dim_mv ds
                                                ON ds.school_dw_id = sl.school_dw_id
                                                    AND trunc(login_local_date_time) >= ds.academic_year_start_date
                            WHERE ds.school_id = '3e9bf712-c13e-43e1-a64f-d5180ea9ee9c' -- keeps only sports academy school
                                  ),
     lessons_started as (SELECT distinct fle_student_dw_id,
                         ...
                         AND ds.school_id = '3e9bf712-c13e-43e1-a64f-d5180ea9ee9c' -- keeps only sports academy school
                         GROUP BY 1),
     holidays_dimension as
         (SELECT DISTINCT cast(holiday_date AS date) AS holiday_date,
                          holiday_organisation_dw_id
          FROM coredw.dim_holiday)
