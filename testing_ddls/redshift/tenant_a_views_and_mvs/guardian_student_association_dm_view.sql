CREATE OR REPLACE VIEW bi_coredw_dev.guardian_student_association_dm_view as
...
FROM coredw.dim_guardian dg
     ...
     (SELECT DISTINCT bst.student_dw_id,
                      ...
                      dse.section_dw_id,
                      dse.section_name
      FROM bi_coredw.bi_student_dim_mv bst
     ...
     (SELECT DISTINCT guardian_dw_id,
      ...
      WHERE guardian_invitation_status = 2
        AND guardian_student_dw_id IS NOT NULL) dgu ON dgu.guardian_dw_id = dg.guardian_dw_id
         ...
         INNER JOIN coredw.dim_tenant dtn ON dtn.tenant_id = dsc.tenant_id
