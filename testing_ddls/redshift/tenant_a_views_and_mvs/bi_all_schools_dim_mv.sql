CREATE MATERIALIZED VIEW bi_coredw_dev.bi_all_schools_dim_mv AUTO REFRESH YES AS
...
WITH school_academic_year AS
         ...
         FROM (SELECT *,
              ...
              FROM coredw.dim_school_academic_year_association sayarn
              INNER JOIN coredw.dim_academic_year ay
                  ON sayarn.saya_academic_year_id = ay.academic_year_id
              WHERE ay.academic_year_status = 1
              QUALIFY rn = 1) saya
         LEFT JOIN coredw.dim_academic_year ay2
...
FROM coredw.dim_school dsc
         ...
         LEFT JOIN (select distinct tag_association_id,
                    ...
                    FROM coredw.dim_tag dt
                    where tag_status = 1
                      and tag_association_attach_status = 1
                    GROUP BY tag_association_id) dtg ON dtg.tag_association_id = dsc.school_id
         LEFT JOIN coredw.dim_tenant dtn
                   ON dtn.tenant_id = dsc.school_tenant_id
         LEFT JOIN coredw.dim_organization org ON dsc.school_organization_dw_id = org.organization_dw_id
WHERE lower(nvl(tag_name, 'NA')) NOT LIKE '%core_test_schools%'
