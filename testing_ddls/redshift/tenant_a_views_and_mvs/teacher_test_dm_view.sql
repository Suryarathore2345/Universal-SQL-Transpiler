CREATE OR REPLACE VIEW bi_coredw_dev.teacher_test_dm_view AS
WITH cte_teachers AS
         (SELECT DISTINCT teacher_dw_id,
                          teacher_id,
                          class_dw_id
          FROM coredw.dim_class dc
                   JOIN coredw.dim_class_user dcu ON dcu.class_user_class_dw_id = dc.class_dw_id
                   LEFT JOIN coredw.dim_teacher dt
                             ON dcu.class_user_user_dw_id = dt.teacher_dw_id
                                 AND dt.teacher_status = 1
                                 AND teacher_id NOT IN
                                     (SELECT DISTINCT teacher_id FROM bi_coredw.exclude_teacher_id)
     ...
     tt_assigned_students AS
         (SELECT DISTINCT ttca_test_id,
          ...
          FROM coredw.dim_teacher_test_candidate_association AS ttca
                   JOIN bi_coredw.bi_student_dim_mv ds
     ...
     dim_teacher_test as
         (SELECT DISTINCT tt.tt_dw_id,
          ...
          FROM coredw.dim_teacher_test tt
                   JOIN coredw.dim_class dc on dc.class_id = tt.tt_test_class_id AND tt.tt_status = 1
                   JOIN bi_coredw.bi_active_schools_dim_mv sc
                   ...
                   JOIN coredw.dim_teacher_test_delivery_settings ds
          ...
          FROM coredw.dim_teacher_test tt
                   ...
                   JOIN bi_coredw.bi_active_schools_dim_mv sc
                        ON fnv_hash(dc.class_school_id) = fnv_hash(sc.school_id)
                            AND trunc(tt_created_time) >= sc.academic_year_start_date
                            AND trunc(tt_created_time) <= sc.academic_year_end_date
                   LEFT JOIN coredw.dim_teacher_test_delivery_settings ds
                             ON fnv_hash(ds.ttds_test_id) = fnv_hash(tt.tt_test_id) AND ttds_status = 1
...
FROM cte_teachers ct
         ...
         LEFT JOIN coredw.fact_teacher_test_candidate_progress fttcp
