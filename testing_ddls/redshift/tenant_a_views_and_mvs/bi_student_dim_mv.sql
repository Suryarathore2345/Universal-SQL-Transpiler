create materialized view bi_coredw.bi_student_dim_mv as
with student_curr_info as
         (select distinct student_dw_id, student_tags, student_special_needs, student_first_created_date
          from (select student_dw_id,
                       CASE
                           WHEN NVL(trim(student_tags), '') <> 'CoreStars' AND
                                NVL(trim(student_tags), '') <> 'Elite' AND
                                NVL(trim(student_tags), '') <> 'Elite, CoreStars'
                               THEN 'Non Elite'
                ...
                from coredw.dim_student dst
                         inner join coredw.dim_school ds on dst.student_school_dw_id = ds.school_dw_id
               )
          where rnk = 1
         )
