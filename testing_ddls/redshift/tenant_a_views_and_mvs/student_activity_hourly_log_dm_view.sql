CREATE OR REPLACE VIEW bi_coredw_dev.student_activity_hourly_log_dm_view AS
WITH current_ay_grade AS (SELECT grade_k12grade, grade_dw_id
                          FROM coredw.dim_grade
                          WHERE academic_year_id IN (SELECT academic_year_id
                                                     FROM bi_coredw.bi_active_schools_dim_mv)
),
     converted_fsta
         AS (SELECT fsta.fsta_created_time,
             ...
             FROM coredw.fact_student_activities fsta
                      INNER JOIN coredw.dim_tenant t
