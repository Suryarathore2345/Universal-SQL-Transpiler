CREATE OR REPLACE VIEW bi_coredw_dev.ip_instructional_plan_dm_view AS
SELECT DISTINCT dsc.tenant_name,
                ...
                cts.teacher_ids,
                'IP' as course_type
FROM bi_coredw.ip_class_total_students_mv cts
         JOIN coredw.dim_instructional_plan dip ON MD5(cts.instructional_plan_id) = MD5(dip.instructional_plan_id)
    ...
    AND instructional_plan_item_optional IS FALSE
         ...
         JOIN bi_coredw.bi_active_schools_dim_mv dsc ON cts.school_dw_id = dsc.school_dw_id
         JOIN coredw.dim_learning_objective dip_dlo ON dip.instructional_plan_item_lo_dw_id = dip_dlo.lo_dw_id
              AND dip_dlo.lo_status=1
    AND nvl(dip_dlo.lo_type, 'NA') <> 'EXPERIENTIAL_LESSON'
         LEFT JOIN bi_coredw.ip_students_lesson_progress_mv lp
         ...
         LEFT JOIN bi_coredw.bi_student_dim_mv ds ON ds.student_dw_id = lp.student_dw_id
