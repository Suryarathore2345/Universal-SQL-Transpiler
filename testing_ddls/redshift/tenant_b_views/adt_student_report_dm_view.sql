CREATE OR REPLACE VIEW eagles_coredw_dev.adt_student_report_dm_view AS
WITH dim_student AS (                 -- get the current school of the student, but keep other attributes at time of test
    ...
    FROM bi_coredw.bi_student_dim_mv ds
    LEFT JOIN bi_coredw.bi_student_dim_mv st
...
dim_test AS (
    ...
    FROM coredw.dim_learning_objective lo
    INNER JOIN coredw.dim_step_instance si
...
adt_potential_students AS (
    ...
    INNER JOIN coredw.dim_course_ability_test_association cata
    ...
    INNER JOIN bi_coredw.bi_active_schools_dim_mv sc
    ...
    INNER JOIN coredw.dim_section sn
        ON sn.section_dw_id = ds.student_section_dw_id
    INNER JOIN coredw.dim_adt_attempt_threshold aat
...
SELECT fasr_dw_id,
         ...
         FROM coredw.fact_adt_student_report
         WHERE fasr_status = 1
     ...
     UNION ALL
        SELECT  dw_id AS fasr_dw_id,
         ...
         FROM coredw.fact_candidate_assessment_progress fcap
...
percentile_adt AS (
    ...
    FROM bi_coredw.adt_attempt1_percentile a1
    FULL OUTER JOIN bi_coredw.adt_attempt2_percentile a2 ON a1.grade = a2.grade and a1.percentile = a2.percentile
    FULL OUTER JOIN bi_coredw.adt_attempt3_percentile a3 ON a1.grade = a3.grade and a1.percentile = a3.percentile
    ),
fact_adt AS (
SELECT fasr.fasr_dw_id,
	...
	CASE lower(fasr_framework)
        WHEN 'core'  THEN  fasr.fasr_final_grade::VARCHAR
	    WHEN 'core scale' THEN  fasr.fasr_final_grade::VARCHAR
    ...
    INNER JOIN bi_coredw.bi_active_schools_dim_mv sch
    ...
    INNER JOIN coredw.dim_grade g
        ON g.grade_dw_id = st.student_grade_dw_id
    LEFT JOIN bi_coredw.adt_cefr_level_mapping_mv cefr
