create or replace view eagles_coredw.guardian_enrollment_p as
...
from (select tst.local_date,
      ...
      from bi_coredw.total_students tst
    ...
    from coredw.dim_guardian dg
             inner join (select distinct st.student_dw_id,
                         ...
                         from bi_coredw.bi_student_dim_mv st
             ...
             inner join bi_coredw.bi_active_schools_dim_mv dsc on dsc.school_dw_id = ds.student_school_dw_id
...
grant delete, insert, references, select, trigger, update on eagles_coredw.guardian_enrollment_p to group business_intelligence;

grant delete, insert, references, select, trigger, update on eagles_coredw.guardian_enrollment_p to group datascience;

grant delete, insert, references, select, trigger, update on eagles_coredw.guardian_enrollment_p to group tdc;

grant select on eagles_coredw.guardian_enrollment_p to group ro_users;