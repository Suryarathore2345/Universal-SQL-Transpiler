...
FROM coredw.dim_date d
       CROSS JOIN bi_coredw.bi_active_schools_dim_mv dsc
       INNER JOIN teacher dt
...
FROM bi_coredw.students_lesson_progress_mv slp
    INNER JOIN coredw.dim_class dc
        ...
        AND LOWER(dc.class_gen_subject) != 'core stars'
    INNER JOIN coredw.dim_course_activity_association dcaa
    ...
    INNER JOIN coredw.dim_learning_objective dip_dlo
...
FROM coredw.dim_class dc
    ...
    INNER JOIN coredw.dim_learning_objective dip_dlo
        ...
        AND nvl(dip_dlo.lo_type, 'NA') <> 'EXPERIENTIAL_LESSON'
        AND COALESCE(lo_template_uuid, 'DISTINCT_VALUE')  NOT IN ('235229fa-4707-4286-8ec2-85f70347096a', '15295fd1-b5e3-46f9-9045-86ee3b13552b')
    INNER JOIN bi_coredw.class_total_students_mv cts
