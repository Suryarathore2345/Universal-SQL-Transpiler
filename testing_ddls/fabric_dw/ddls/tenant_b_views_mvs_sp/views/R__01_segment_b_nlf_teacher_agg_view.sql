CREATE OR ALTER VIEW ${OS_EAGLES_COREDW}.pathway_nlf_teacher_agg_view
AS
WITH pathway_class_total_students AS (
    ...
    INNER JOIN ${RS_COREDW}.dim_section AS sc
...
adaptive_practice_fact AS (
    SELECT DISTINCT
        ...
        ap.class_dw_id,
        ap.pathway_dw_id,
        CONVERT(DATE, ap.created_time) AS ap_created_date
...
skills_active_fact AS (
    SELECT DISTINCT
        ...
        slp.class_dw_id,
        slp.material_dw_id,
        CONVERT(DATE, slp.created_time) AS saf_created_date
    FROM ${RS_COREDW}.fact_pathway_skill_learning_progress AS slp
...
skills_started_fact AS (
    SELECT DISTINCT
        ...
        slp.class_dw_id,
        slp.material_dw_id,
        CONVERT(DATE, slp.created_time) AS ssf_created_date
    FROM ${RS_COREDW}.fact_pathway_skill_learning_progress AS slp
    WHERE event_type = 'SkillExperienceStarted'
),

skills_gaps_introduced AS (
    SELECT
        student_dw_id,
        class_dw_id,
        pathway_dw_id,
        CONVERT(DATE, sgt.created_time) AS sgi_created_date,
        COUNT(DISTINCT skill_dw_id) AS skill_gaps
    FROM ${RS_COREDW}.fact_pathway_skill_gap_tracker AS sgt
    WHERE status = 'INTRODUCED'
      AND NOT EXISTS (
          SELECT 1
          FROM ${RS_COREDW}.fact_pathway_skill_gap_tracker AS sgt2
    ...
    GROUP BY
        student_dw_id,
        class_dw_id,
        CONVERT(DATE, sgt.created_time),
        pathway_dw_id
),

students_agg AS (
    SELECT DISTINCT
        COALESCE(ssf.ssf_created_date, saf.saf_created_date, ap.ap_created_date) AS created_date,
...
JOIN students_agg_monthly AS sda
    ON pcts.pathway_student_dw_id = sda.started_student_dw_id
   AND pcts.class_dw_id = sda.class_dw_id
   AND sda.activity_month
       BETWEEN DATETRUNC(month, academic_year_start_date) AND DATETRUNC(month, academic_year_end_date)
LEFT JOIN ${RS_EAGLES_COREDW}.pathway_class_teacher AS pct
