CREATE OR ALTER VIEW ${os_bi_coredw}.guardian_login_activity_dm_view AS
         ...
         (select distinct dg.guardian_dw_id,
                          ...
                          first_value(dg.guardian_created_time)
                                      over (partition by dg.guardian_dw_id
                                          order by dg.guardian_created_time ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) guardian_registered_date
          from ${rs_coredw}.dim_guardian dg
               ...
               (select school_dw_id, sum(total_students) as total_students
                ...
                where local_date = CONVERT(DATE, GETDATE() - 1)
...
from guardian_info gi
         left join (select distinct fgaa_guardian_dw_id,
                                    ...
                                    CONVERT(VARCHAR, YEAR(dt.academic_year_start_date)) + '-' +
                                    CONVERT(VARCHAR, YEAR(dt.academic_year_end_date))                               AS academic_year,
                                    CONVERT(
                                        DATE,
