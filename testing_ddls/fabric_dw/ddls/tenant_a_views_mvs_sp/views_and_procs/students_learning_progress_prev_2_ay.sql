CREATE OR ALTER PROCEDURE ${os_bi_coredw}.usp_refresh_students_learning_progress_prev_2_ay
AS
BEGIN
    SET NOCOUNT ON;
    BEGIN TRY
        ...
        -- --------------------------------------------------------
        DROP TABLE IF EXISTS ${os_bi_coredw}.students_learning_progress_prev_2_ay_staging;

        -- --------------------------------------------------------
        -- Step 2: CTAS - Build staging table with full transformation
        -- --------------------------------------------------------
        CREATE TABLE ${os_bi_coredw}.students_learning_progress_prev_2_ay_staging
        ...
        WITH school_previousay AS (
            ...
            FROM (
                SELECT academic_year_id, academic_year_school_id, academic_year_dw_id,
                    ...
                    ROW_NUMBER() OVER (PARTITION BY academic_year_school_id ORDER BY academic_year_end_date DESC) AS rank
                FROM ${rs_coredw}.dim_academic_year
                WHERE academic_year_is_roll_over_completed = 1
                  AND academic_year_school_id = 'c97921a8-6555-4c5f-acd2-9487803f2b66'
            ) AS pr_ay
            WHERE pr_ay.rank = 2
        ),
        COMPLETED_LESSONS AS (
            ...
            FROM (
                ...
                JOIN ${rs_coredw}.dim_learning_objective AS lo ON lo.lo_dw_id = fle_lo_dw_id
                JOIN school_previousay AS ay ON fle_academic_year_dw_id = ay.academic_year_dw_id
                WHERE fle_completion_node = 1
                  AND fle_activity_type <> 'INTERIM_CHECKPOINT'
                  AND fle_material_type <> 'PATHWAY'
                  AND fle_is_additional_resource <> 1
            ) AS subq
            WHERE rnk = 1
        ),
        LESSON_PROGRESS AS (
            ...
            FROM (
                ...
                WHERE fle_ls_id NOT IN (SELECT fle_ls_id FROM COMPLETED_LESSONS)
                  ...
                  AND fle_material_type <> 'PATHWAY' AND fle_is_additional_resource <> 1
        ...
        student_lessons_assigned AS (
            ...
            WHERE d_cu.class_user_status = 1
              AND d_cu.class_user_role_dw_id = 2
              AND dc.class_course_status = 'CONCLUDED'
              AND dc.class_school_id = 'c97921a8-6555-4c5f-acd2-9487803f2b66'
        )
        ...
        FROM (
            SELECT DISTINCT
                ...
                fle_lesson_category, fle_dw_id,
                dst.student_dw_id, dst.student_section_dw_id, fle_academic_year_dw_id,
                ...
                previous_academic_year_start_date, previous_academic_year_end_date
            ...
            JOIN student_lessons_assigned AS dcu ON fle_student_dw_id = dcu.class_user_user_dw_id AND fle_lo_dw_id = dcu.lo_dw_id
        ...
        -- --------------------------------------------------------
        DROP TABLE IF EXISTS ${os_bi_coredw}.students_learning_progress_prev_2_ay;

        -- --------------------------------------------------------
        -- Step 4: Promote staging to production
        -- --------------------------------------------------------
        EXEC sp_rename '${os_bi_coredw}.students_learning_progress_prev_2_ay_staging', 'students_learning_progress_prev_2_ay';
