CREATE OR ALTER VIEW ${os_bi_coredw}.principal_login_activity_ay_view AS
WITH total_principals AS (
    SELECT DISTINCT
        ds.school_dw_id,
        ds.school_id,
        ds.school_name,
    ...
    CROSS JOIN (
        ...
        WHERE full_date BETWEEN CONVERT(DATE, '2024-07-10')
    ...
    INNER JOIN ${rs_coredw}.dim_staff_user dp
       ...
       AND (
            (
                (dp.staff_user_status = 2 AND dp.staff_user_enabled = 1)
                AND d.full_date >= CONVERT(
                    DATE,
            ...
            OR (dp.staff_user_status = 1 AND dp.staff_user_enabled = 1)
       )

    WHERE dsusra.susra_role_dw_id = 6
      AND d.full_date BETWEEN ds.academic_year_start_date
...
active_principals AS (
    ...
    FROM (
        ...
        INNER JOIN ${rs_coredw}.dim_staff_user dp1
           ...
           AND (
                (
                    (dp1.staff_user_status = 2 AND dp1.staff_user_enabled = 1)
                ...
                OR (dp1.staff_user_status = 1 AND dp1.staff_user_enabled = 1)
           )
        WHERE CONVERT(DATE, pl.login_local_date_time) BETWEEN CONVERT(DATE, '2024-07-10')
                                                         AND CONVERT(DATE, GETDATE())
    ) prin_dateset
),
