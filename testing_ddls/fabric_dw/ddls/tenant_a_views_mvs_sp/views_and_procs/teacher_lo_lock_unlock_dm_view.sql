CREATE OR REPLACE VIEW bi_coredw_dev.teacher_lo_lock_unlock_dm_view AS
...
FROM coredw.dim_content_student_association dcsa
JOIN bi_coredw.bi_student_dim_mv sdm ON MD5(sdm.student_id) = MD5(dcsa.content_student_association_student_id)
JOIN bi_coredw.bi_active_schools_dim_mv sch ON sch.school_dw_id = sdm.student_school_dw_id
JOIN coredw.dim_class dc ON MD5(dc.class_id) = MD5(dcsa.content_student_association_class_id)
JOIN coredw.dim_academic_year ay ON MD5(ay.academic_year_school_id) = MD5(sch.school_id)
JOIN bi_coredw.class_total_students_mv cts ON cts.class_dw_id = dc.class_dw_id
JOIN coredw.dim_learning_objective dlo ON MD5(dlo.lo_id) = MD5(dcsa.content_student_association_lo_id)
