CREATE OR ALTER PROCEDURE ${os_bi_coredw}.usp_refresh_core_course_ic_experience
AS
BEGIN
    SET NOCOUNT ON;

    BEGIN TRY

        -- --------------------------------------------------------
        -- Step 1: Drop stale staging table if it exists
        -- --------------------------------------------------------
        DROP TABLE IF EXISTS ${os_bi_coredw}.core_course_ic_experience_staging;

        -- --------------------------------------------------------
        -- Step 2: CTAS - Build staging table with full transformation
        -- --------------------------------------------------------
        CREATE TABLE ${os_bi_coredw}.core_course_ic_experience_staging
        WITH (CLUSTER BY (fle_class_dw_id, fle_lo_dw_id))
        AS
        SELECT
            fle.fle_class_dw_id,
            ISNULL(fle.fle_section_dw_id, 10001)                AS fle_section_dw_id,
            fle.fle_lo_dw_id,
            COUNT(DISTINCT fle.fle_student_dw_id)               AS total_students_fact,
            COUNT(DISTINCT CASE
                WHEN fle.fle_completion_node = 1
                    THEN fle.fle_student_dw_id
            END)                                                AS total_completed_students,
            COUNT(DISTINCT CASE
                WHEN fle.fle_completion_node = 1
                 AND fle.fle_total_score >= 70
                    THEN fle.fle_student_dw_id
            END)                                                AS meets_completed_students,
            COUNT(DISTINCT CASE
                WHEN fle.fle_completion_node = 1
                 AND fle.fle_total_score >= 50
                 AND fle.fle_total_score < 70
                    THEN fle.fle_student_dw_id
            END)                                                AS approaching_completed_students,
            COUNT(DISTINCT CASE
                WHEN fle.fle_completion_node = 1
                 AND fle.fle_total_score < 50
                 AND fle.fle_total_score >= 0
                    THEN fle.fle_student_dw_id
            END)                                                AS below_completed_students,
            SUM(CASE
                WHEN fle.fle_completion_node = 1
                    THEN fle.fle_total_score
            END)                                                AS fle_score,
            SUM(CASE
                WHEN fle.fle_total_time <= 1200 THEN fle.fle_total_time
                WHEN fle.fle_total_time > 1200  THEN 1200
                ELSE 0
            END)                                                AS session_time
        FROM ${rs_coredw}.fact_learning_experience AS fle
        JOIN ${rs_coredw}.dim_interim_checkpoint AS ic
            ON ic.ic_dw_id = fle.fle_lo_dw_id
           AND ic.ic_status = 1
        WHERE fle.fle_abbreviation <> 'NA'
          AND fle.fle_activity_type = 'INTERIM_CHECKPOINT'
          AND fle.fle_material_type <> 'PATHWAY'
          AND fle.fle_ls_id NOT IN (
                SELECT DISTINCT fle_ls_id
                FROM ${rs_coredw}.fact_learning_experience
                WHERE fle_state = 4
          )
        GROUP BY
            fle.fle_class_dw_id,
            ISNULL(fle.fle_section_dw_id, 10001),
            fle.fle_lo_dw_id;

        -- --------------------------------------------------------
        -- Step 3: Drop existing production table
        -- --------------------------------------------------------
        DROP TABLE IF EXISTS ${os_bi_coredw}.core_course_ic_experience;

        -- --------------------------------------------------------
        -- Step 4: Promote staging to production
        -- --------------------------------------------------------
        EXEC sp_rename '${os_bi_coredw}.core_course_ic_experience_staging', 'core_course_ic_experience';

    END TRY
    BEGIN CATCH
        THROW;
    END CATCH
END;
