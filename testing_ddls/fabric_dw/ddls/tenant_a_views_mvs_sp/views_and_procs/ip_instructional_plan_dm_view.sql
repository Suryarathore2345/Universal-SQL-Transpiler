CREATE OR ALTER VIEW ${os_bi_coredw}.ip_instructional_plan_dm_view AS
SELECT DISTINCT dsc.tenant_name,
                ...
                cts.teacher_ids,
                'IP' as course_type
FROM ${rs_bi_coredw}.ip_class_total_students cts
         JOIN ${rs_coredw}.dim_instructional_plan dip ON cts.instructional_plan_id = dip.instructional_plan_id
    AND dip.instructional_plan_status = 1
    AND instructional_plan_item_optional = 0
         JOIN ${rs_coredw}.dim_week dw ON dip.instructional_plan_item_week_dw_id = dw.week_dw_id
         JOIN ${rs_coredw}.dim_term dtrm ON dw.week_term_id = dtrm.term_id
         JOIN ${rs_bi_coredw}.bi_active_schools_dim dsc ON cts.school_dw_id = dsc.school_dw_id
         JOIN ${rs_coredw}.dim_learning_objective dip_dlo ON dip.instructional_plan_item_lo_dw_id = dip_dlo.lo_dw_id
              AND dip_dlo.lo_status=1
    AND ISNULL(dip_dlo.lo_type, 'NA') <> 'EXPERIENTIAL_LESSON'
         LEFT JOIN ${rs_bi_coredw}.ip_students_lesson_progress lp
                   ON cts.class_dw_id = lp.fle_class_dw_id
                       AND cts.section_dw_id = lp.student_section_dw_id
                       AND cts.class_curriculum_id = lp.term_curriculum_id
                       AND dip.instructional_plan_item_lo_dw_id = lp.lo_attempted
         LEFT JOIN ${rs_bi_coredw}.bi_student_dim ds ON ds.student_dw_id = lp.student_dw_id;
