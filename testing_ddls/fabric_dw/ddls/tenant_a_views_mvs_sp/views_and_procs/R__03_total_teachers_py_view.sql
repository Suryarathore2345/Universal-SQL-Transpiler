CREATE OR ALTER VIEW ${os_bi_coredw}.total_teachers_py_view AS
WITH date_dimension AS (
    ...
    WHERE full_date = DATEADD(day, -1, CONVERT(DATE, GETDATE()))
),
...
SELECT DISTINCT
    ...
    CONVERT(VARCHAR(4), NULL) AS org_term,
...
FROM (
    ...
    WHERE d.local_date BETWEEN s.academic_year_start_date
                          AND s.academic_year_end_date
) ds
