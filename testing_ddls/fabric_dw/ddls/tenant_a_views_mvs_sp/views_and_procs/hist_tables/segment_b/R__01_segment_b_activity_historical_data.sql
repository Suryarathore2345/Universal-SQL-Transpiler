DROP TABLE IF EXISTS ${os_bi_coredw}.pathway_activity_historical_data;

CREATE TABLE ${os_bi_coredw}.pathway_activity_historical_data AS
...
pathway_adt_students AS (
    SELECT DISTINCT
        ...
        fle.fle_school_dw_id AS fle_school_dw_id,
...
prev_ay AS (
    ...
    WHERE academic_year_is_roll_over_completed = 1
      AND YEAR(academic_year_end_date) >= (
          SELECT MAX(YEAR(academic_year_end_date)) - 2
          FROM ${rs_bi_coredw}.bi_all_schools_dim
      )
),
...
SELECT DISTINCT
    ...
    CASE
        ...
        WHEN (flr.flr_recommendation_type < 5 AND pcts.curr_subject_name <> 'Science')
...
LEFT JOIN pathway_adt_students pnadt
   ...
   AND pnadt.adt_created_time BETWEEN
       pcts.academic_year_start_date AND pcts.academic_year_end_date;