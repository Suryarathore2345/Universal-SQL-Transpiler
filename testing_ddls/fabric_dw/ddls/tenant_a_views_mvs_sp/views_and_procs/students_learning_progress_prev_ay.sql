CREATE OR ALTER PROCEDURE ${os_bi_coredw}.usp_refresh_students_learning_progress_prev_ay
AS
BEGIN
    SET NOCOUNT ON;
    ...
    BEGIN TRY
        ...
        -- --------------------------------------------------------
        DROP TABLE IF EXISTS ${os_bi_coredw}.students_learning_progress_prev_ay_staging;

        -- --------------------------------------------------------
        -- Step 2: CTAS - Build staging table with full transformation
        -- --------------------------------------------------------
        CREATE TABLE ${os_bi_coredw}.students_learning_progress_prev_ay_staging
        ...
        SELECT
            ...
            fl.fle_lesson_category, fl.fle_dw_id, fl.student_dw_id, fl.student_section_dw_id,
            fl.fle_academic_year_dw_id, fl.student_tags, fl.student_special_needs,
            fl.grade_k12grade, fl.session_time, fl.previous_academic_year_start_date,
            fl.previous_academic_year_end_date, lps.fle_score, lps.lo_status
        FROM (
            SELECT DISTINCT
        ...
        -- --------------------------------------------------------
        DROP TABLE IF EXISTS ${os_bi_coredw}.students_learning_progress_prev_ay;

        -- --------------------------------------------------------
        -- Step 4: Promote staging to production
        -- --------------------------------------------------------
        EXEC sp_rename '${os_bi_coredw}.students_learning_progress_prev_ay_staging', 'students_learning_progress_prev_ay';
