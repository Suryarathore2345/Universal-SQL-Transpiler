CREATE OR ALTER VIEW ${os_bi_coredw}.dag_student_login_military_historical_data AS

WITH calendar_year AS (
    SELECT DISTINCT full_date
    FROM ${rs_coredw}.dim_date
    WHERE full_date <= CONVERT(DATE, DATEADD(day, -1, GETDATE()))
      AND full_date >= DATEFROMPARTS(YEAR(GETDATE()) - 5, 1, 1)
),
...
login_data AS (
    ...
    WHERE sch.school_organization_dw_id = 29
      AND (CONVERT(DATE, sl.login_local_date_time) BETWEEN ay.academic_year_start_date AND ay.academic_year_end_date)
),
...
reg_data AS (
    SELECT DISTINCT
        ...
        ay.organisation_dw_id                              AS content_repository_dw_id,
        ay.school_organisation                             AS content_repository_name,
        CONVERT(DATE, full_date)                            AS local_date,
        t.tenant_name,
        g.grade_dw_id,
        g.grade_name
