CREATE OR ALTER VIEW ${os_bi_coredw}.dag_teacher_stats_monthly AS
...
login_data AS (
    ...
    WHERE
        CONVERT(DATE, tl.login_local_date_time) BETWEEN ay.academic_year_start_date AND ay.academic_year_end_date
        AND NOT EXISTS (SELECT 1 FROM ${rs_bi_coredw}.exclude_teacher_id excl WHERE excl.teacher_id = dt.teacher_id)  -- OPT-8
        AND DATEADD(month, DATEDIFF(month, 0, tl.login_local_date_time), 0) >=
...
FROM (
    ...
    WHERE
        (
            cy.calendar_month_start_date = DATEADD(month, DATEDIFF(month, 0, dt.teacher_created_time), 0)
            OR cy.calendar_month_end_date = DATEADD(month, DATEDIFF(month, 0, dt.teacher_active_until), 0)
            OR (
                cy.calendar_month_start_date >= DATEADD(month, DATEDIFF(month, 0, dt.teacher_created_time), 0)
                AND cy.calendar_month_end_date <= DATEADD(month, DATEDIFF(month, 0, dt.teacher_active_until), 0)
            )
            OR (
                dt.teacher_active_until IS NULL
                AND dt.teacher_status = 1
                AND cy.calendar_year_end_date >= DATEADD(month, DATEDIFF(month, 0, dt.teacher_created_time), 0)
            )
        )
        AND (
            sch.school_status = 1
            OR DATEADD(month, DATEDIFF(month, 0, sch.school_updated_time), 0) >= cy.calendar_month_start_date
        )
