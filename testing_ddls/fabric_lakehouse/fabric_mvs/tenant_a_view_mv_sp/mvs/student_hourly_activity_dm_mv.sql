CREATE OR REPLACE MATERIALIZED LAKE VIEW {{os_bi_coredw}}.student_hourly_activity_dm_mv
AS
SELECT DISTINCT
    CAST(
        from_utc_timestamp(
            sl.ful_created_time,
            COALESCE(dsc.tenant_timezone, 'UTC')
        ) AS DATE
    ) AS local_date,
...
WHERE sl.ful_role_dw_id = 2
  AND CAST(sl.ful_created_time AS DATE) >= CAST(date_add(current_date(), -90) AS DATE)
  AND CAST(
        from_utc_timestamp(
            sl.ful_created_time,
            COALESCE(dsc.tenant_timezone, 'UTC')
...
GROUP BY
    ...
    CAST(year(dsc.academic_year_start_date) AS VARCHAR(4)) || '-' ||
    CAST(year(dsc.academic_year_end_date) AS VARCHAR(4)),
    dse.section_dw_id,
