CREATE OR REPLACE MATERIALIZED LAKE VIEW {{os_bi_coredw}}.pathway_adaptive_practice_mv
AS
...
pathway_adt_students AS (
    -- getting student's test last record by same learning session and attempt
    SELECT DISTINCT
        fasr_student_dw_id AS adt_student_dw_id,
        fasr_created_time  AS adt_created_time,
        INITCAP(fasr_class_subject_name) AS class_gen_subject
    FROM {{rs_coredw}}.fact_adt_student_report AS fasr
    WHERE fasr_status = 1
      AND CAST(fasr_created_time AS DATE) >= DATE '2025-07-01'

    UNION

    SELECT DISTINCT
        candidate_dw_id AS adt_student_dw_id,
        created_time    AS adt_created_time,
        INITCAP(
            CASE
                WHEN framework = 'CEFR' THEN 'Arabits'
                ELSE subject
            END
        ) AS class_gen_subject
    FROM {{rs_coredw}}.fact_candidate_assessment_progress AS fcap
    WHERE event_type = 'CandidateReportGeneratedDataEvent'
      AND academic_year_tag = '2025-2026'

    UNION

    -- students with no ADT report but ADT completed
    SELECT DISTINCT
    ...
    WHERE fle.fle_lesson_category = 'DIAGNOSTIC_TEST'
      AND fle.fle_exp_ls_flag IS FALSE
),

-- unique adaptive practice events
adaptive_practive_events AS (
    SELECT DISTINCT
        CAST(created_time AS DATE) AS local_date,
        ...
        FIRST_VALUE(time_spent) OVER (
            PARTITION BY student_dw_id, level_dw_id, level_proficiency_tier
        ...
        answer_score,
        CASE
            WHEN level_proficiency_score >= 0.75 THEN level_dw_id
        END AS level_completed_dw_id,
        CASE
            WHEN level_proficiency_score >= 0.75 THEN
                FIRST_VALUE(created_time) OVER (
    ...
    FROM {{rs_coredw}}.fact_adaptive_practice_progress AS ap
    WHERE event_type = 'AdaptivePracticeAnswerSubmittedEvent'
),

fact_adaptive_practice AS (
    SELECT DISTINCT
        apel.*
    FROM adaptive_practive_events AS apel
    WHERE (apel.student_dw_id, apel.level_dw_id, apel.skill_dw_id, apel.question_id, apel.attempt_number) IN (
        SELECT
            student_dw_id,
            level_dw_id,
            skill_dw_id,
...
fact_pathway_placement AS (
    SELECT DISTINCT
        flr_created_time        AS fpp_created_time,
        ...
        SELECT
            ...
            ROW_NUMBER() OVER (
                PARTITION BY flr_student_dw_id,
                             flr_course_activity_container_dw_id,
                             flr_academic_year
                ORDER BY flr_created_time
            ) AS rank
        FROM {{rs_coredw}}.fact_levels_recommended
        WHERE flr_recommendation_type NOT IN (2, 3)
          AND flr_academic_year = '2025-2026'
    ) AS flr
...
skill_proficiency_update AS (
    SELECT DISTINCT
        ...
        status,
        CAST(created_time AS DATE) AS update_date
    FROM {{rs_coredw}}.fact_student_proficiency_tracker
    WHERE event_type = 'SkillProficiencyUpdatedEvent'
),

level_proficiency_update AS (
    SELECT DISTINCT
        student_dw_id,
        level_dw_id,
        academic_year_tag,
...
skill_gap_tracker AS (
    SELECT
        ...
        array_join(
            transform(
                array_sort(collect_list(struct(created_time, status))),
                x -> x.status
            ),
            ','
        ) AS skill_gap_status,
        array_join(
            transform(
                array_sort(collect_list(struct(created_time, created_time AS ct))),
                x -> x.ct
            ),
            ','
        ) AS skill_gap_created_time
    FROM (
        SELECT DISTINCT
            student_dw_id,
            skill_dw_id,
            ...
            created_time,
            ROW_NUMBER() OVER (
                PARTITION BY student_dw_id, skill_dw_id, status
                ORDER BY created_time
            ) AS skill_gap_number
        FROM {{rs_coredw}}.fact_pathway_skill_gap_tracker
    ) AS sgt
    GROUP BY
        student_dw_id,
        skill_dw_id,
...
SELECT DISTINCT
    ...
    CONCAT(
        YEAR(pcts.academic_year_start_date),
        '-',
        YEAR(pcts.academic_year_end_date)
    ) AS academic_year,
    dcr.course_dw_id,
    ...
    CAST(pnadt.adt_created_time AS DATE) AS adt_completed_date,
    fpp.fpp_student_dw_id,
    fpp.fpp_course_dw_id,
    fpp.fpp_placement_type,
    CAST(fpp.fpp_created_time AS DATE) AS placement_date,
    path.local_date AS pathway_activity_date,
    path.local_date_time AS pathway_activity_time,
    from_utc_timestamp(path.local_date_time, pcts.tenant_timezone)
        AS pathway_activity_date_local,
    path.pathway_dw_id,
    path.class_dw_id,
    path.student_dw_id,
    path.level_dw_id,
    ...
    dcr.course_placement_type,
    path.level_completed_dw_id,
    path.level_completed_date,
    from_utc_timestamp(path.level_completed_date, pcts.tenant_timezone)
        AS level_completed_date_local,
    path.answer_score,
    path.question_id,
    path.attempt_number,
...
FROM pathway_class_total_students AS pcts
LEFT JOIN {{rs_coredw}}.dim_course AS dcr
    ON pcts.class_material_id = dcr.course_id
   AND dcr.course_status = 1
   AND dcr.course_type = 'PATHWAY'
LEFT JOIN fact_pathway_placement AS fpp
...
LEFT JOIN level_proficiency_update AS lpu
   ...
   AND path.activity_type = 'ADAPTIVE PRACTICE'
LEFT JOIN skill_gap_tracker AS sgt
    ON sgt.student_dw_id = path.student_dw_id
   AND sgt.skill_dw_id = path.activity_dw_id
   AND path.activity_type = 'ADAPTIVE PRACTICE'
LEFT JOIN {{rs_coredw}}.dim_course_activity_container AS dcac
...
LEFT JOIN pathway_adt_students AS pnadt
    ON pcts.pathway_student_dw_id = pnadt.adt_student_dw_id
   AND lower(pcts.curr_subject_name) = lower(pnadt.class_gen_subject)
   AND CAST(pnadt.adt_created_time AS DATE)
       BETWEEN pcts.academic_year_start_date AND pcts.academic_year_end_date;