CREATE MATERIALIZED LAKE VIEW {{os_bi_coredw}}.agg_student_learning_progress_mv
AS
...
students_progress AS (
    SELECT
        ...
        MAX(CASE
                WHEN slp.lo_status = 'Completed' AND slp.fle_score >= 0
                    THEN CAST(slp.fle_score AS DECIMAL(10,2))
    ...
    JOIN {{rs_coredw}}.dim_learning_objective dip_dlo
       ...
       AND COALESCE(dip_dlo.lo_template_uuid, 'DISTINCT_VALUE') NOT IN (
