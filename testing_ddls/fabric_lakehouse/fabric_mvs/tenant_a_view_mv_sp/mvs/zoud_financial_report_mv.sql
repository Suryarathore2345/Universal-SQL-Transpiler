CREATE OR REPLACE MATERIALIZED LAKE VIEW {{os_bi_coredw}}.zoud_financial_report_mv AS
WITH learning_objective AS (
    SELECT
        lo_dw_id,
        lo_code,
        lo_title,
        -- DYNAMIC ID GENERATION (Using SPLIT function)
        CASE
            WHEN lo_code LIKE 'FL_G%' THEN
                ...
                (CAST(REPLACE(SPLIT(lo_code, '_')[1], 'G', '') AS INT) * 10000)
                + CAST(REPLACE(SPLIT(lo_code, '_')[2], 'L', '') AS INT)
            ...
            PARTITION BY
                CASE
                    WHEN lo_code LIKE 'FL_G%' THEN
                        ...
                        CONCAT(
                            REPLACE(SPLIT(lo_code, '_')[1], 'G', ''),
                            '.',
                            CAST(CAST(REPLACE(SPLIT(lo_code, '_')[2], 'L', '') AS INT) AS STRING)
                        )
                END
...
class_total_students_zoud_literacy AS (
    SELECT 
...
completed_lessons AS (
    ...
    FROM (
        SELECT 
            ...
            CAST('Completed' AS STRING) AS lo_status,
            fle_total_score,
            ROW_NUMBER() OVER (
