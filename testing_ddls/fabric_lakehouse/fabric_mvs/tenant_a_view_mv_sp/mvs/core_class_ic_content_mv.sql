CREATE or Replace MATERIALIZED LAKE VIEW {{os_bi_coredw}}.core_class_ic_content_mv
AS
WITH ip_base AS (
    SELECT
        course_dw_id,
...
SELECT DISTINCT
    ...
    DENSE_RANK() OVER (
        PARTITION BY dc.class_dw_id,
                     dc.class_material_id,
                     dc.class_gen_subject
        ORDER BY dpg.pacing_activity_order
    )                                                                       AS ic_order,
...
INNER JOIN {{rs_coredw}}.dim_interim_checkpoint AS ic
...
LEFT JOIN (
    ...
    FROM {{rs_coredw}}.dim_interim_checkpoint_rules
    WHERE ic_rule_status = 1
    GROUP BY ic_rule_ic_dw_id
) AS icr
...
WHERE dcr.course_status = 1
  ...
  AND (
        (ip_course_check.course_dw_id IS NOT NULL
         ...
         AND dpg.pacing_activity_dw_id IS NOT NULL) --  If course exists in ip_course_check, then the specific activity must also have a match in the main 'ip' join and it should be in pacing
        ...
        OR (ip_course_check.course_dw_id IS NOT NULL
            AND ip_activity_check.activity_dw_id IS NULL) -- If course exists in ip_course_check, but activity does not exists in IP - include those activities
      )
...
SELECT 
    ...
    academic_year
FROM {{rs_bi_coredw}}.core_ip_class_ic_content;
